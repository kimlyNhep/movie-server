...
let movieResolvers = class movieResolvers {
    async updateMovieInfo(options) {
        let info = await typeorm_1.getConnection()
            .createQueryBuilder()
            .select('info')
            ...
            .getOne();
        if (!info) {
            const movie = await Movie_1.Movie.findOne({ where: { id: options.movie } });
            if (!movie) {
                return {
                    errors: [
                        {
        ...
        if (options.characters) {
            characters = await typeorm_1.getRepository(User_1.User).findByIds(options.characters);
        ...
        try {
            ...
            await queryRunner.manager.update(MovieInfo_1.MovieInfo, info.id, info);
            const errors = await class_validator_1.validate(info);
            if (errors.length > 0) {
                return {
    ...
    async updateMovie(options) {
        const movie = await Movie_1.Movie.findOne({ where: { id: options.id } });
        if (!movie) {
            return {
                errors: [
                    {
        ...
        const genres = await typeorm_1.getRepository(Genre_1.Genre).findByIds(options.genres);
        ...
        movie.genres = genres;
        const errors = await class_validator_1.validate(movie);
        if (errors.length > 0) {
            return {
                errors: errors.map((error) => {
                    const { constraints, property } = error;
    ...
    async createMovieInformation(options) {
        const movie = await Movie_1.Movie.findOne({ where: { id: options.movie } });
        if (!movie) {
            return {
                errors: [
        ...
        let characters;
        if (options.characters) {
            characters = await typeorm_1.getRepository(User_1.User).findByIds(options.characters.map((item) => item.id));
            if (characters.length < options.characters.length)
                return {
                    errors: [
                        {
        ...
        try {
            ...
            const errors = await class_validator_1.validate(movieInfo);
            if (errors.length > 0) {
                return {
                    errors: errors.map((error) => {
                        const { constraints, property } = error;
    ...
    async getMovie(id) {
        const movieQuery = await typeorm_1.getConnection()
            .createQueryBuilder()
            .select('movie')
    ...
    async getMovies() {
        const moviesQuery = await typeorm_1.getConnection()
            .createQueryBuilder()
            .select('movie')
            .from(Movie_1.Movie, 'movie')
            .innerJoinAndSelect('movie.creator', 'creator')
