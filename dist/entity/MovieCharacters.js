"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.MovieCharacters = void 0;
const type_graphql_1 = require("type-graphql");
const MovieInfo_1 = require("./MovieInfo");
const typeorm_1 = require("typeorm");
const User_1 = require("./User");
let MovieCharacters = class MovieCharacters {
};
__decorate([
    type_graphql_1.Field(() => User_1.User),
    typeorm_1.JoinColumn({ name: 'userId' }),
    typeorm_1.ManyToOne(() => User_1.User, (user) => user.movieCharacters, { primary: true }),
    __metadata("design:type", User_1.User)
], MovieCharacters.prototype, "characters", void 0);
__decorate([
    type_graphql_1.Field(() => MovieInfo_1.MovieInfo),
    typeorm_1.JoinColumn({ name: 'movieinfoId' }),
    typeorm_1.ManyToOne(() => MovieInfo_1.MovieInfo, (info) => info.movieCharacters, { primary: true }),
    __metadata("design:type", MovieInfo_1.MovieInfo)
], MovieCharacters.prototype, "movieInfo", void 0);
__decorate([
    type_graphql_1.Field({ nullable: true }),
    typeorm_1.Column({ nullable: true }),
    __metadata("design:type", String)
], MovieCharacters.prototype, "role", void 0);
MovieCharacters = __decorate([
    type_graphql_1.ObjectType(),
    typeorm_1.Entity('movie_characters')
], MovieCharacters);
exports.MovieCharacters = MovieCharacters;
//# sourceMappingURL=MovieCharacters.js.map